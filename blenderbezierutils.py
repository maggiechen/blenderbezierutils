...
class IntersectCurvesMeshCleanerOp(Operator):
    ...
    def traverse_mesh(self, mesh, cross_section_point_count, cross_section_horizontal_size, min_point_index, max_point_index):
        ...
        def get_loop_starting_at_vert_index(in_index, in_face):
            for loop in in_face.verts[in_index].link_loops:
                if loop.face == in_face and loop.vert == face.verts[in_index]:
                    return loop
            for i, face_loop in enumerate(face.loops):
                if face_loop == loop:
                    return i
            return None

        loop_extreme_1_index = get_loop_starting_at_vert_index(horizontal_span_index_1, face)
        loop_extreme_2_index = get_loop_starting_at_vert_index(horizontal_span_index_2, face)
        ...
        while len(edge_loop_in_ring.face.loops) != cross_section_point_count:
            ...
            while edge_loop_in_ring.index != starting_loop_index or first_execution:
                ...
                if loop_extreme_1_index == edge_loop_index:
                    current_extreme_1_vert = edge_loop_in_ring.vert
                if loop_extreme_2_index == edge_loop_index:
    ...
    def execute(self, context):
        ...
        cross_section_object = selected_objects[-1]
        mesh_points = cross_section_object.to_mesh()
        bm_points = bmesh.new()
