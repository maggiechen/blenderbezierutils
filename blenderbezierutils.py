...
def intersectCurves(curves, action, firstActive, margin, rounding):
    ...
    if(action in {'INSERT_PT', 'CUT'}):
        ...
        for i, key in enumerate(mapKeys):
            intersectPts, _, _ = zip(*intersectMap[key])
...
class IntersectCurvesMeshCleanerOp(Operator):
    ...
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = "Clean up the meshes generated by the the selected curves at their intersections, using the cross section used to generate the meshes."
    bl_cross_section_axis = "x"
    ...
    def find_sequence_index(self, list_to_search, sequence_to_find):
        """
        Assumes sequence_to_find is longer than size 1
        Assumes the sequence_to_find *can* be found in list_to_search
        """
        for current_index, current_val in enumerate(list_to_search):
            if not isclose(current_val, sequence_to_find[0], abs_tol=self.absolute_float_tolerance):
                continue

            found = True
            for match_index in range(1, len(sequence_to_find)):
                if not isclose(list_to_search[current_index + match_index], sequence_to_find[match_index], abs_tol=self.absolute_float_tolerance):
                    found = False
            if found:
    ...
    def traverse_mesh(self, blender_mesh, cross_section_point_count, cross_section_horizontal_size, min_point_index, max_point_index, cross_section_points_list, out_point_pairs):
        ...
        first_face_heights = [loop.vert.co.__getattribute__(self.bl_mesh_up_axis) for loop in face.loops]
        ...
        while len(edge_loop_in_ring.face.loops) != cross_section_point_count:
            ...
            current_extreme_1_vert = None
            current_extreme_2_vert = None
            vert_index_in_ring = 0
            while edge_loop_in_ring.index != starting_loop_index or first_execution:
                if first_execution:
                    first_execution = False

                if loop_extreme_1_index == vert_index_in_ring:
                    current_extreme_1_vert = edge_loop_in_ring.vert
                if loop_extreme_2_index == vert_index_in_ring:
                    current_extreme_2_vert = edge_loop_in_ring.vert
    ...
    def get_ordered_cross_section_points(self, points):
        ...
        while len(todo) > 0:
            curr_point = todo.pop()
            vertex_index = curr_point.index
            seen.add(vertex_index)
            for edge in curr_point.link_edges:
                do_outer_break = False
    ...
    def execute(self, context):
        ...
        # generate pairs of bezier curves and meshes
        curve_mesh_pairs = []
        curve_list = []
        for object_index in range(0, len(selected_objects) - 1, 2):
            first = selected_objects[object_index]
            second = selected_objects[object_index + 1]
        ...
        input_points = bm_points.verts
        input_points.ensure_lookup_table()

        points = self.get_ordered_cross_section_points(input_points)
...
class BezierUtilsPanel(Panel):
    ...
    def draw(self, context):
        ...
        if(context.mode == 'OBJECT'):
            ...
            row = layout.row()
            row.prop(params, "intersectMeshCleanerExpanded",
                icon="TRIA_DOWN" if params.intersectMeshCleanerExpanded else "TRIA_RIGHT",
                icon_only=True, emboss=False
            )
            row.label(text='Intersect Curves Mesh Cleaner', icon='GRAPH')
            if params.intersectMeshCleanerExpanded:
                box = layout.box()
                col = box.column().split()
                row = col.row()
                row.prop(params, 'intersectMargin', text = 'Proximity')
                ...
                row = col.row()
                col = box.column().split()
                col.operator('object.intersect_curves_mesh_cleaner')
...
def getIntersectPts(seg0, seg1, soln, solnRounded, recurs, margin, rounding, \
    maxRecurs = 100, t0_lo = 0.0, t1_lo = 0.0, t_range = 1.0):
    ...
    if(overlap):
        ...
        if(all(abs(bbox0[0][i] - bbox0[1][i]) < margin for i in range(3))):
            ...
            if(roundedVect not in solnRounded):
                soln.append((center, t0_lo + t_change, t1_lo + t_change))
            ...
            r0 = getIntersectPts(seg01, seg11, soln, solnRounded, \
                recurs + 1, margin, rounding, t0_lo = t0_lo, t1_lo = t1_lo, t_range = t_change)
            r1 = getIntersectPts(seg01, seg12, soln, solnRounded, \
                recurs + 1, margin, rounding, t0_lo = t0_lo, t1_lo = t1_lo + t_change, t_range = t_change)
            r2 = getIntersectPts(seg02, seg11, soln, solnRounded, \
                recurs + 1, margin, rounding, t0_lo = t0_lo + t_change, t1_lo = t1_lo, t_range = t_change)
            r3 = getIntersectPts(seg02, seg12, soln, solnRounded, \
                recurs + 1, margin, rounding, t0_lo = t0_lo + t_change, t1_lo = t1_lo + t_change, t_range = t_change)
...
class BezierToolkitParams(bpy.types.PropertyGroup):
    ...
    intersectMeshCleanerExpanded: BoolProperty(name = "Intersect Curves Mesh Cleaner", default = False)
...
# ==================================================
# From https://blender.stackexchange.com/a/241035
